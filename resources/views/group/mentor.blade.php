      ...
      <div class="card-body py-10">
        <div class="d-flex align-items-center justify-content-between mb-9">
          ...
          <div class="modal fade" id="kt_modal_add_user" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-sm mw-400px">
              <div class="modal-content">
                <div class="modal-header" id="kt_modal_add_user_header">
                  <h2 class="fw-bolder">Add New Mentor</h2>
                  ...
                  <form  class="form" action="{{ route('group.mentor.store', $group->id) }}" method="POST">
                    ...
                    <select class="form-select" placeholder="Select an option" id="kt_docs_select2_country" name="mentorId" required>
                      @foreach ($allMentors as $item)
                        <option value="{{ $item->id }}" data-kt-select2-country="@if($item->image) {{ $item->image}} @else https://ui-avatars.com/api/?background=000C32&color=fff&name={{ $item->name }} @endif">{{ $item->name }}</option>
                      @endforeach
                  </select>
                ...
                <tr>
                  <td class="d-flex align-items-center">
                    <div class="symbol-group symbol-hover me-3">
                      <div class="symbol symbol-45px symbol-circle" data-bs-toggle="tooltip" title="{{ $item->mentor->name }}">
                        <img src="@if($item->mentor->image) {{ $item->mentor->image}} @else https://ui-avatars.com/api/?background=000C32&color=fff&name={{ $item->mentor->name }} @endif" alt="">
                      </div>
                    </div>
                    <div class="d-flex flex-column">
                      <span class="text-gray-800 mb-1">{{ $item->mentor->name }}</span>
                      <span>{{ $item->mentor->email }}</span>
                    </div>
                  </td>
                  <td>
                    {{ $item->mentor->phone_number }}
                  </td>
                  <td>
                    {{ $item->created_at}}
                  </td>
